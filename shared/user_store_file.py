...
class UserStoreFile:
    ...
    def __str__(self):
        return f"UserStoreFile({self.user_file_name})"
    ...
    def create_user(self, user_name: str, password: str, role: str):
        ...
        new_user = User(user_name=user_name, password_hash=password_hash, role=role)
        users.append(new_user)
        self.write_users(users)

    def find_user_by_name(self, user_name) -> Iterable[User]:
        user_list = self.read_users()
        return [profile for profile in user_list if profile.user_name == user_name]
